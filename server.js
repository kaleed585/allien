...
setInterval(() => {
    http.get(`http://khbc18.glitch.me/`); /// حط اسم المشروع تبعك name تعديل مهم بدل
}, 280000);
...
client.login("NzE0OTcxOTc5Mjc0MjU2NTM1.XwkfaA.sKLUtDcmfqz0fTZF0v_VWLFLqPU");
client.on('ready', () => {
...
client.on("message", async message => {
    ...
    if (message.content.startsWith(prefix + "bc")) {
        ...
        let args = message.content
            ...
            .slice(1)
            .join(" ");
        if (!message.author.id === "701273494318481420") return; ///تعديل مهم حط الايدي تبعك
...
client.on("ready", () => {
    client.user.setActivity(`>`, { ///تعديل مهم حالة البوت
        type: "WATCHING"
    }); ///تعديل حالة البوت
});
